...
accounts.get("/id/:id", authenticateToken, async (req, res) => {
    ...
    const user = await UserModel.findById(id).select({ username: 1, _id: 0 });
...
accounts.get("/:name", async (req, res) => {
    ...
    for (let i = 0; i < allProblems.length; i++) {
        if (allProblems[i].main.difficulty === "easy") {
            easyProblems++;
            if (user.problems_solved.includes(allProblems[i].main.name_slug)) {
                easySolved++;
            }
    ...
    // Avoid duplicate views by useEffect
    user.views += 0.5;
    await user.save();
});
