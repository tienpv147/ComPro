...
const ProblemPage = ({
    ...
    const handleChangeLanguage = (newLang: React.SetStateAction<{ id: number; name: string; label: string; value: string; }>) => {
    ...
    const [relatedProblems, setRelatedProblems] = useState<RelateProblemData>(
        {
            topic_tags: [],
            similar_questions: [],
        }
    ...
    const submitCode = (currentLangValue: string) => {
        ...
        axios
            .post<
                {},
                { data: Submission[] },
                { code: string; id: string; problem_name: string; current_lang: string; }
            >(`${API_URL}/api/problem/submit/${name}`, {
                code,
                id,
                problem_name,
                current_lang: currentLangValue,
            })
            .then(({ data }) => {
                setIsSubmitted(true);
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then(({ data }) => {
                if (activeNavOption === "hints") {
                    setHints(data);
                }
                if (activeNavOption === "related-problems") {
                ...
                >
                    ...
                    >
                        ...
                        <div className="description-body relative w-full h-[calc(100%-50px)] overflow-y-auto bg-black">
                            ...
                            {activeNavOption === "related-problems" ? (
                                <RelatedProblem data={relatedProblems}/>
                            ) : activeNavOption === "related-problems" ? (
                                <Loading For="pRelatedProblems" />
                            ) : (
                                <></>
                            )}
                    ...
                    <div className="flex flex-col h-[calc(100%-16px)] min-w-[calc(20%-8px)] mr-[8px] flex-grow">
                        <div className="min-h-0 flex-grow min-w-full mr-[8px] mb-[8px] rounded-lg bg-black border border-borders">
                            <div className="h-[50px] bg-black relative border-b border-borders">
                                ...
                                {/*</div>*/}
                                <LanguagesDropdown onSelectChange={handleChangeLanguage}/>
                            </div>
                            <ReactCodeMirror
                                value={
                                    code === "" || code == null
                                ...
                                extensions={[loadLanguage(currentLang.value as LanguageName)!]}
                                theme={tokyoNight}
                                onChange={(value) => {
                                    setCode(value);
                                }}
                        ...
                        <div
                            id="console"
                            className="flex justify-end items-center bg-black w-full h-[50px] rounded-lg border border-borders"
                        >
                            <div
                                ...
                                onClick={() => submitCode(currentLang.value)}
                            >
                                {isSubmitLoading ? (
